...
function authcode($string,$key) {
    $key = $key?$key:C('CODE_KEY');
    $string=$string.$key;
    return sha1(md5(crypt($string,$key)));
}
/*************************************************解析json为数组**********************************************/
...
/*************************************************生成随机数**********************************************/
function rand_num($len=8,$format='NUMBER'){
    $is_abc = $is_numer = 0;
    $password = $tmp ='';
    switch($format){
        case 'ALL':
...
/*************************************************获取市*********************************************/
function get_shiqu($provice){
    $data=get_address();
    foreach ($data as $key=>$value) {
        $value = object_to_array($value);
        $i = 0;
        if ($provice == $value['name']) {
            foreach ($value['sub'] as $key => $value) {
                $address[$i]=$value['name'];
                $i++;
            }
        }
    }
...
/*************************************************获取区*********************************************/
function get_diqu($provice,$shiqu){
    $data=get_address();
    foreach ($data as $key=>$value) {
        $value = object_to_array($value);
        if ($provice == $value['name']) {
...
{
    $_arras= is_object($obj) ? get_object_vars($obj) : $obj;
    foreach($_arras as $key=> $val)
    {
...
//获取图片详细地址
function getTrueImgSrc($src,$thum=false,$height=300,$width=300){
    //$thum是否是缩略图
    if($thum){
        $true_src=C('OSS_WEB_URL').$src."?x-oss-process=image/resize,h_".$height.",w_".$width;
    }else{
        $true_src=C('OSS_WEB_URL').$src;
    }
...
/*************************************************************下载excel表格*****************************************/
function downExcel($data,$title,$name='煤问题'){
    /*
     *$data:要下载的数据数组
     * $title:生成的表头
     */
    $acb_arr=range('A','Z');
    {
        ...
        $objPHPExcel->getProperties()->setCreator("煤问题")
            ->setLastModifiedBy("煤问题")
    ...
    foreach ($title as $key=>$value){
        ...
        $objPHPExcel->getActiveSheet()
            ->setCellValue($acb_arr[$key].'1',$value);
        //背景颜色
        $objPHPExcel->getActiveSheet()->getStyle($acb_arr[$key].'1')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID)
            ->getActiveSheet()->getStyle($acb_arr[$key].'1')->getFill()->getStartColor()->setARGB('FF808080')
            ->getActiveSheet()->getStyle($acb_arr[$key].'1')->getFont()->setBold(true);
        //宽度
        $objPHPExcel->getActiveSheet()->getColumnDimension($acb_arr[$key])->setAutoSize(true);
    }
    ...
    foreach ($data as $key=>$value){
        ...
        foreach ($value as $k=>$v){
            $objPHPExcel->getActiveSheet()->setCellValue($acb_arr[$i] .$key ,$v);
            $i++;
        }
    ...
    //文件名称
    header('Content-Disposition: attachment;filename='.$name.date('m/d').'.xlsx');
...
// 1、简单方法+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
 * debug 方法，输出最后一条sql语句
 */
function sql(){
    echo M()->_sql();exit;
}
/**
 * debug 方法，记录数据到文本，但不停止程序
 */
function dump_text($var){
    $content = '';
    if (is_string($var)) {
        $content = $var;
        ...
        foreach ($var as $key => $val) {
            if (is_array($val)) {
                ...
                $content .= '<pre>' . htmlspecialchars($output, ENT_QUOTES) . '</pre>';
            } else {
                $content .= $key .'->'.$val.',';
            }
        }
    ...
    $old_content = file_get_contents(LOG_PATH.'/zgw_debug.txt');
    if (strlen($old_content) > 100000) {
        $old_content = substr($old_content,0,99799);
    }

    $new_content = date('Y-m-d H:i:s').PHP_EOL.$content.PHP_EOL.$old_content;
    file_put_contents(LOG_PATH.'/zgw_debug.txt', $new_content);
}
function dump_text_exit($var){
    dump_text($var);exit;
}
/**
 * debug 方法，没有权限时新增
 * $type  0是普通方法，1是菜单，加载页面
 * $is_admin 0 vip  ,1 admin
 */
function addRule($rule, $type=0, $is_admin = 0){
    $data = array('name' => $rule);
    if ($type == 1) {
        $data['title'] = '加载页面';
    } else {
        $data['status'] = 0;
    ...
    if ($is_admin == 0) {
        ...
        $rule_table = M('admin_auth_rule');
    }

    $res = $rule_table->where(array('name' => $rule))->find();
    // sql();
    if (!$res) {
        $auth_id = $rule_table->add($data);
    } else {
        $auth_id = $res['id'];
    }

    if ($is_admin == 0) {
        $auth_list = M('users')->where('id = '.session('user_id'))->getField('auth_list');
    } else {
        $info = M('admin u')->field('g.id as gid, g.rules')
            ->join('left join coal_admin_auth_group_access aga on aga.uid = u.id')
            ->join('left join coal_admin_auth_group g on g.id = aga.group_id')
            ->where(array('u.id' => session('sys_uid')))
            ->find();
        $auth_list = $info['rules'];
    }
    ...
    $arr = explode(',', $auth_list);
    $arr1 = array_unique($arr);
    $auth_list1 = implode(',', $arr1);
    if ($is_admin == 0) {
        M('users')->where('id = '.session('user_id'))->save(array('auth_list' => $auth_list1));
    } else {
        M('admin_auth_group')->where('id = '.$info['gid'])->save(array('rules' => $auth_list1));
    }

    // D('Users', 'Logic')->loginout(); // 添加菜单（循环）时关闭，其他时候（单个）打开
}
//自定义分页
//function page($count, $where, $rows=15){
 ...
 * @param $code
 */
function alert($msg, $code = 200){
    $array = array('statusCode' => $code, 'message' => $msg);
    echo json_encode($array);exit;
}

/**
 * alert_false  传递信息到前台页面,简单错误处理
 * $msg string 错误信息
 */
function alert_false($msg='处理失败'){
    $array = array('statusCode' => 300, 'message' => $msg);
    echo json_encode($array);exit;
}
/**
 * alert_false  传递信息到前台页面,非法访问
 */
function alert_illegal(){
    $array = array('statusCode' => 300, 'message' => '非法访问');
    echo json_encode($array);exit;
}
/**
 * after_alert  传递信息到前台页面,
 * @param $msg
 * @param $code
 */

function after_alert($options, $code = 200){
    if ($code == 200) {
        $msg = '处理成功';
    } else {
        $code = 300;
    ...
    $msg = isset($options['message'])?$options['message']:$msg;
...
/**
 * show_res 快速告诉前台处理结果
 * @param  boolean $result 一个用于判断成功或失败的值
 * @param  string $remark 额外提示信息
 * @return
 */
function show_res($result, $remark = ''){
    if ($result !== false) {
        alert('处理成功！');
    } else {
        alert('处理失败！'.$remark,300);
    }
    return;
}
/**
 ...
 * @param  [type] $type   类型，dialog/doajax
 * @param  [type] $title  按钮名
 * @return [type]
 */
function create_button($option, $type, $title){
    return '<button type="button" class="btn-default" data-toggle="'.$type.'" data-options="'.$option.'">'.$title.'</button>';
}
...
/**
 * get_work_time_type 得到当前时间是白班还是夜班
 * @return int 2：白班，3：夜班
 */
function get_work_time_type(){
...
/**
 * 保存订单的gps，地图坐标
 * @param $begin_gps 带,的GPS坐标
 * @param $end_gps
 * @param $record_id order记录的id
 * @return mixed
 */
function save_order_gps($begin_gps, $end_gps, $record_id){
    $begin_gps = str_replace(',',' ',$begin_gps);
    $end_gps = str_replace(',',' ',$end_gps);
    $sql="update coal_orders SET `begin_gps`= ST_GeomFromText('POINT(".$begin_gps.")'),`end_gps`=ST_GeomFromText('POINT(".$end_gps.")') WHERE (id=$record_id)";
 ...
 * @return mixed
 */
function getComapnyGps($company_id){
    $sql="select ST_X(add_gps) as x,ST_Y(add_gps) as y from coal_company WHERE (id=".$company_id.")";
    $gps=M()->query($sql);
    return $gps[0];
}
/**
 * 更新公司地图坐标
 * @param $add_gps 带,的GPS坐标
 * @param $record_id 公司记录的id
 * @return mixed
 */
function update_company_gps($add_gps, $record_id){
    $add_gps = str_replace(',',' ',$add_gps);
    $sql="update coal_company SET `add_gps`= ST_GeomFromText('POINT(".$add_gps.")') WHERE (id=$record_id)";
    $res = M('company')->execute($sql);
    return $res;
}
// 显示V标
function show_vip_icon($name, $is_vip = -1){
    if ($is_vip = -1) {
        $is_vip = M('company')->where(array('name' => $name))->getField('is_vip');
    }
    if ($is_vip == 1) {
        // 认证过的企业
        $icon = '<span style="color:red">V</span>';
    } else if ($is_vip == 0) {
        // 个体户
    }
    return $icon.$name;
 ...
 * @return array
 */
function get_subcompany_ids($parent_id){
    // 子公司信息
    $sub_company = M('company')->where(array('pid' => $parent_id))->select();
...
/**
 * 根据是否总公司返回对应的公司id或包含子公司的ids
 * @param $company_id
 * @return array
 */
function retrun_company_ids($company_id){
    if (is_parent_company($company_id)) {
        $sub_ids = get_subcompany_ids($company_id);
        return array_merge(array($company_id),$sub_ids);
    } else {
...
/**
 * 通过公司id得到公司名字，显示在页面上或使用
 * @param int $id  公司id
 * @return string  公司名
 */
function getCompanyName($id){
    $name = M('company')->where(array('id' => $id))->getField('name');
    return $name;
}

// 是否总公司
function is_parent_company($company){
    $info = M('company')->find($company);
    return $info['pid'] == 0?true:false;
}

// 拿到总公司管理员的所有权限(不给子公司创建分公司的权限)
function getAuthListAll($not_in = array(25)){
    //获得所有菜单的节点
    $one = M('menu')->where(array('is_admin' => 0, 'pid' => 0))->select();
    if ($one) {
        foreach ($one as $key => $value) {
            $two = M('menu')->where(array('is_admin' => 0, 'pid' => $value['id'], 'id' => array('notin', $not_in)))->select();
            if ($two) {
                $one[$key]['list'] = $two;
...
// 判断是否是待认领的公司
function is_company_have_manage($company_id){
    $res = M('users')->where(array('company_id' => $company_id, 'is_admin' => 1))->find();
    return $res;
}

// ++++++++ 公司管理员
//得到公司管理员的号码
function get_company_phone($company){
    if (!is_integer($company)) {
        $company = M('company')->where(array('name' => $company))->getField('id');
    }
    $phone = M('users')->where(array('company_id' => $company, 'is_admin' => 1))->getField('phone');
    if (!$phone) {
        $phone = '没有管理员';
    }
    return $phone;
...
// ++++++++ 车主
// 通过车辆id得到车主的名字
function get_trucker_name($truck_id){
    $truck_info = M('truck')->find($truck_id);
    if ($truck_info['owner_type'] == 1) {
        $real_name = M('users')->where(array('id' => $truck_info['owner_id']))->getField('real_name');
    } else {
        // 公司
        if ($truck_info['owner_id'] == session('company_id')) {
            $real_name = '自有车辆';
        } else {
            $real_name = M('company')->where(array('id' => $truck_info['owner_id']))->getField('name');
        }
    }
    return $real_name;
}

// 得到车主的名字和手机,通过车辆id得到手机号
function get_trucker_name_and_phone($truck_id){
    $truck_info = M('truck')->find($truck_id);
    $res = array();
    if ($truck_info['owner_type'] == 1) {
        $res1 = M('users')->find($truck_info['owner_id']); // 个人
        $res['type'] = 1;
        $res['trucker_name'] = $res1['real_name'];
        $res['phone'] = $res1['phone'];
    } else {
        $res1 = M('company')->find($truck_info['owner_id']);// 公司
        // 公司管理员信息
        $res2 = M('users')->where(array('company_id' => $truck_info['owner_id'], 'is_admin' => 1))->find();
        $res['type'] = 2;
        $res['company_name'] = $res1['name']; // 公司名字
        $res['trucker_name'] = $res2?'(公司)'.$res2['real_name']:$res1['name'];
        $res['phone'] = $res1['phone'];
    }
    return $res;
}
...
// ++++++++ 车辆
// 得到车辆的来源（合作车主、自有、合作公司）
function get_trcuk_from($truck_id){
    $truck_info = M('truck')->find($truck_id);
    if ($truck_info['owner_type'] == 1) {
        $from = '合作车主';
    } else {
...
// 得到车辆上次的gps
function getTruckGps($truck_id){
    $res = M('bill')->where(array('truck_id' => $truck_id, 'driver_id' => array('neq', 0)))->order('id desc')->find();
    if (!$res) {
        return '没有坐标';
    } else {
        $gps = M('gps_history')->where(array('uid' => $res['driver_id']))->order('id desc')->find();
        if ($gps) {
            return '('.$gps['x'].','.$gps['y'].')';
        } else {
            return '没有坐标';
...
// ++++++++ 司机
// 通过车辆id得到已审核过司机的名字
function get_drivers($truck_id){
    $drivers = M('driver dr')
        ->field('dr.*, u.real_name,u.phone')
        ->join('left join coal_users u on u.id = dr.uid')
        ->where(array('dr.truck_id' => $truck_id, 'dr.is_passed' => 1))
        ->select();
    $drivers_count = count($drivers);
    if ($drivers_count == 0) {
        $driver_name = '还没有绑定司机';
    } else if ($drivers_count == 1) {
        $driver_name = $drivers[0]['real_name'].'('.$drivers[0]['phone'].')';
    } else {
        $driver_name = '';
        foreach ($drivers as $val) {
            $driver_name .= $val['real_name'].'('.$val['phone'].')<br>';
        }
    }
    return $driver_name;
}
...
function get_user_id($account){
    $uid = M('users')->where(array('account' => $account, 'phone' => $account, '_logic' => 'or'))->getField('id');
    return $uid;
}
 ...
 * 验证菜单是否有权限显示
 * @param $name            菜单标识符
 * @param $cur_auth_list   当前用户的权限
 * @return bool
 */
function isMenuCanShow($name, $cur_auth_list){
    $auth_id = M('auth_rule')->where(array('name' => $name))->getField('id');
    // echo $auth_id,$cur_auth_list;exit;
    if ($auth_id > 0 && in_array($auth_id, explode(',', $cur_auth_list))) {
        // echo 3;exit;
        return true;
    } else {
         // echo 4;exit;
 ...
 * @return bool
 */
function checkVipAuth($name){
    if (!session('is_check_auth')) {
        return true;
    }
    // 获取权限id
    $auth = M('auth_rule')->where(array('name' => $name, 'state' => 1))->find();
    if ($auth) {
        if ($auth['status'] == 1) {
            // 获取当用户的权限
            $s_cur_auth_list_name = 'cur_auth_list_'.session('user_id');
            $cur_auth_list = S($s_cur_auth_list_name);
            if (!$cur_auth_list) {
                $cur_auth_list = M('users')->where('id = '.session('user_id'))->getField('auth_list');
                S($s_cur_auth_list_name, $cur_auth_list, 300);
            }
            if (in_array($auth['id'], explode(',', $cur_auth_list))) {
                return true;
            } else {
                return false;
            }
        ...
        // 2017年5月8日11:01:31 zgw edit 可能有问题。false改为true,原因是我们只控制菜单，这样不是菜单的都不需要增加rule了。
        return true;
    }
}

// 通过vip auth_list 权限字符拿到1、2级菜单
function getMenuListByAuthList($auth_list){
    $one=array();
...
// 编辑时保证字段唯一
function edit_unique($table, $value, $field, $new_value){
    // 如果现值和原值一样，返回真
    if (trim($value) == trim($new_value)) {
        return true;
    } else {
        $res = $table->where("`" . trim($field) . "`='" .trim($new_value) . "'")->find();
...
// 获取物流公司与车主合作的状态
function getCompanyTruckerType($type){
    $msg = '';
    switch ($type) {
        case 1:
            $msg = '已确认';
...
// 获取物流公司与公司合作的状态
function getCompanyCompanyType($type){
    $msg = '';
    switch ($type) {
        case 1:
            $msg = '已确认';
...
// 获取退单提煤单状态
function getReturnBillType($type){
    $msg = '';
    switch ($type) {
        case 1:
            $msg = '待物流公司审核';
 ...
 * @return string
 */
function mwt_base64_encode($data, $key = 'MWTAPP20170607'){
    vendor('Base64.Base64');
    $base64=new \base64();
    $value = $base64->encode($key.$data);
    return $value;
}
 ...
 * 显示伊泰时间
 * @param $time  系统时间
 * @param int $type 增加还是减少
 */
function yitai_time($time, $type = 1){
    ...
    return date('Y-m-d H:i:s', strtotime($a.'6 hours',strtotime($time)));
}
// 是否有车辆没有未执行的计划
function is_truck_cron($truck_id, $deal_source=''){
    $res = M('cron_change_truck_state')->where(array('truck_id' => $truck_id, 'state' => 0))->find();
    if ($res) {
        M('truck')->save(array('id' => $truck_id, 'state' => $res['truck_state']));
        M('cron_change_truck_state')->save(array('id' => $res['id'], 'state' => 1, 'deal_source' => $deal_source));
    }
}
...
*/
//获取天数
function getday($time_str){
    if(!$time_str or $time_str=='0000-00-00 00:00:00'){
        return '未知';
    }
    $time=time()-strtotime($time_str);
//    dump($time);
    if($time<0){
        return '未知';
    }
    ...
    $minit=floor(($time-($day*86400)-$hour*3600)/60);
    if($day){
        $work_time=$day.'天'.$hour.'小时'.$minit.'分钟';
    }else{
        if ($hour){
            $work_time=$hour.'小时'.$minit.'分钟';
        }else{
            if ($minit){
            $work_time=$minit.'分钟';
            }else{
...
function show_weight($weight){
    $weight = ($weight + 0.00);
    if ($weight <  1000) {
        $weight2 = round($weight,2).'吨';
    } else  {
        $weight2 = round(($weight/10000),2).'万吨';
    }
    return $weight2;
}
//获取煤种
function coal_type($type)
{
    return M('coal_type')->where(array('id'=>$type))->getField('name');
}
//通知提煤单信息
function noticeBill($truck_id){
    //查看有哪些司机需要收到短信
    //正在上班并且没有休息的司机才能收到短信
    ...
    foreach ($driver as $key=>$value){
        if(D("Driver",'Logic')->is_work($value['uid'])){
            //推送
            $phone=M('users')->where(array('id'=>$value['uid']))->getField('phone');
            $name=M('users')->where(array('id'=>$value['uid']))->getField('real_name');
...
function decrypt1($data, $key)
{
    ...
    $x = 0;
    dump($data);
    $data = base64_decode($data);//本函数将以 MIME BASE64 编码字符串解码。解码后的字符串可能为中文字符串或其它的二进位资料。
    echo $data;
...
//获取买家是否存在
function getOrderCompanyId($company_name){
...
//新增提煤单派单记录
function addArrHistory($first_truck,$lic_number,$bill_number,$bill_id,$arr_time,$company_id){
    $data['lic_number']=$lic_number;    $data['bill_number']=$bill_number;

    $data['company_id']=$company_id;

    $data['bill_id']=$bill_id;
    $data['last_time']=M('truck')->where(array('lic_number'=>$lic_number))->getField('last_time');

    $data['first_truck']=$first_truck;
    $data['arr_time']=$arr_time;    M('arr_history')->add($data);
}
// 定义一个函数获取客户端IP地址
function getIP(){
    global $ip;
